...
export async function generateMetadata({
  ...
  if (!priceMeta) return fallbackPageMetadata("Best FPL DEFCON Midfielders | ChatFPL AI", "/fpl/defcon/midfielders/x")
  const data = await getDefconPriceHub("midfielders", price)
  const gw = data?.gw ?? "?"
  const title = `Best FPL DEFCON Midfielders Under ${priceMeta.label} for Gameweek ${gw} | ChatFPL AI`
  const description = `Fantasy Premier League midfielders priced at or below ${priceMeta.label} ranked by DEFCON returns per 90 minutes for Gameweek ${gw}. Full data, fixture context and analysis.`
  return buildPageMetadata({ title, description, path: `/fpl/defcon/midfielders/${price}` })
}

export default async function DefconMidfielderPricePage({
  params,
}: {
  params: Promise<{ price: string }>
}) {
