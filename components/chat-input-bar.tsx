...
export const ChatInputBar = forwardRef<ChatInputBarHandle, ChatInputBarProps>(
  function ChatInputBar(
    ...
    const { supported, listening, toggle, start, stop } = useSpeechInput({
      onListenEnd,
    })
    ...
    useImperativeHandle(
      ...
      () => ({
        startListening: () => {
          if (!disabled) start(value, onChange)
        },
        stopListening: () => stop(),
      }),
      [disabled, start, stop, value, onChange],
    )
    ...
    const placeholderText =
      voiceMode && supported
        ? "Tap the mic and ask your FPL question..."
        : placeholder
