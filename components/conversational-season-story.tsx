...
export type SeasonStoryChatItem = {
  id: string
  question: string
  answer: string
}
...
function welcomeText(gw: number, leagueName: string, provisional?: boolean) {
  const leagueBit = leagueName ? ` in ${leagueName}` : ""
  const liveBit = provisional
    ? " Scores are still live, so this preview may shift before the gameweek locks."
    : ""
  return `Gameweek ${gw}${leagueBit}. Tap a question below and I will answer from your league data.${liveBit}`
}

export function ConversationalSeasonStory({
  gw,
  leagueName,
  items,
  provisional,
  ...
  gw: number
  leagueName: string
  items: SeasonStoryChatItem[]
  provisional?: boolean
}) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: "welcome", role: "assistant", text: welcomeText(gw, leagueName, provisional) },
  ])
  ...
  function ask(item: SeasonStoryChatItem) {
  ...
  const remaining = items.filter((item) => !asked.has(item.id))
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              ...
              <div
                className={`max-w-[80%] rounded-[20px] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                  msg.role === "user"
